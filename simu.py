...
for lam in tqdm.tqdm(range(0,11,1),desc="a:"):
    for b in tqdm.tqdm(range(1,11,1),desc="b:"):
        args = []
        time = 0
        while time<100:
            G = nx.barabasi_albert_graph(200, 5)
            deal_unconnected(G)
            args.append([G, 0.2, 0.3, 0.4, 0.3, 1, lam*0.1, b, 0.5, 100,10000])
        ...
        with open('results_strong_ba.txt', 'a') as f:
